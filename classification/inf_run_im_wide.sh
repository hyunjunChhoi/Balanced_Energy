...
data_models=(cifar10_wrn0.01im cifar100_wrn0.01im)
gpu=1
...
if [ "$1" = "MSP" ]; then
	...
	model=${array[1]}
		...
		gamma2=0.50
	            ...
	            echo "---------------"$m_in"------"$m_out"--------------------"
	            CUDA_VISIBLE_DEVICES=$gpu python inf_train_im_wide.py $data --model $model --score $score --seed $seed --m_in $m_in --m_out $m_out --gamma1 $gamma1 --gamma2 $gamma2 --lamb $lamb --trial $3
	            CUDA_VISIBLE_DEVICES=$gpu python inf_test_SC_wide.py --method_name ${dm}_s${seed}_$1 --num_to_avg 1 --score $score --trial $3 --gamma1 $gamma1 --gamma2 $gamma2 
        done
    done
